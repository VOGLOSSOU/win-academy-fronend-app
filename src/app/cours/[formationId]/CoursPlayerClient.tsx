  ...
  return (
    <div style={S.root}>
      {!isDesktop && sidebarOpen && (
        <div onClick={() => setSidebarOpen(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 20 }} />
      )}
      ...
      <div style={sidebarStyle}>
        <div style={{ height: '56px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid #f3f4f6' }}>
          <Link href={`/formations/${formationId}`} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#6b7280', fontSize: '14px', textDecoration: 'none' }}>
        ...
        <div style={{ padding: '16px', borderBottom: '1px solid #f3f4f6', flexShrink: 0 }}>
          <p style={{ fontSize: '12px', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Formation</p>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {formation.modules.map((module, modIdx) => (
            <div key={module.id || modIdx} style={{ borderBottom: '1px solid #f3f4f6' }}>
              <button onClick={() => toggleModule(modIdx)} style={{ width: '100%', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                ...
                <span style={{ flex: 1, fontSize: '13px', fontWeight: 500, color: '#1f2937', lineHeight: 1.3 }}>{module.title}</span>
                {expandedModules.has(modIdx) ? <ChevronUp size={14} color="#9ca3af" /> : <ChevronDown size={14} color="#9ca3af" />}
              </button>
              {expandedModules.has(modIdx) && (
                <div style={{ backgroundColor: '#f9fafb', paddingBottom: '4px' }}>
                  {(module.contents || []).map(c => {
                    ...
                    return (
                      <button key={c.id} onClick={() => navigateTo(c.id)} style={{ width: '100%', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '12px', background: active ? '#eff6ff' : 'none', border: 'none', borderLeft: active ? '3px solid #2563eb' : '3px solid transparent', cursor: 'pointer', textAlign: 'left' }}>
                        {done ? <CheckCircle size={14} color="#22c55e" style={{ flexShrink: 0 }} /> : <Circle size={14} color={active ? '#3b82f6' : '#d1d5db'} style={{ flexShrink: 0 }} />}
                        <span style={{ fontSize: '13px', lineHeight: 1.3, color: active ? '#1d4ed8' : done ? '#9ca3af' : '#374151', fontWeight: active ? 600 : 400, textDecoration: done ? 'line-through' : 'none' }}>
                          {c.title}
                        </span>
                      </button>
                    );
      ...
      <div style={S.contentArea}>
        <div style={S.topBar}>
          {!isDesktop && (
            <button onClick={() => setSidebarOpen(true)} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer', borderRadius: '8px' }}>
              <Menu size={20} />
            </button>
          )}
          ...
          {allDone && formation.evaluation && (
            <Link href={`/evaluation/${formation.evaluation.id}?enrollmentId=${enrollment.id}&formationId=${formationId}`} style={{ padding: '6px 16px', backgroundColor: '#16a34a', color: 'white', borderRadius: '8px', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', textDecoration: 'none', flexShrink: 0 }}>
              <Award size={15} />Quiz final
            </Link>
          )}
        </div>
          ...
          {loadingContent ? (
            ...
            <div style={S.contentWrapper}>
              {currentContent.module?.title && (
                <p style={{ fontSize: '13px', color: '#9ca3af', fontWeight: 500, marginBottom: '8px' }}>{currentContent.module.title}</p>
              )}
              <h2 style={S.lessonTitle}>{currentContent.title}</h2>
              <div style={S.card}>
                {currentContent.type === 'TEXT' && (
                  <div style={S.bodyText}>{currentContent.body || 'Contenu non disponible pour le moment.'}</div>
                )}
                ...
                {currentContent.type === 'PDF' && currentContent.url && (
                  <div>
                    ...
                    <a href={currentContent.url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', color: '#2563eb', fontSize: '14px', marginTop: '12px', textDecoration: 'none' }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginBottom: '40px' }}>
                <button onClick={() => { if (currentIdx > 0) navigateTo(allContents[currentIdx - 1].id); }} disabled={currentIdx <= 0} style={{ padding: '14px 24px', border: '2px solid #e5e7eb', borderRadius: '12px', background: 'white', color: '#4b5563', fontWeight: 600, fontSize: '15px', cursor: currentIdx <= 0 ? 'not-allowed' : 'pointer', opacity: currentIdx <= 0 ? 0.4 : 1 }}>
                  ← Précédent
                </button>
                <button onClick={markAsComplete} style={{ flex: 1, padding: '14px', borderRadius: '12px', fontWeight: 700, fontSize: '15px', cursor: 'pointer', border: isCurrentDone ? '2px solid #bbf7d0' : 'none', backgroundColor: isCurrentDone ? '#f0fdf4' : '#2563eb', color: isCurrentDone ? '#15803d' : 'white' }}>
                  {isCurrentDone ? (isLastLesson ? '✓ Formation terminée' : '✓ Continuer →') : (isLastLesson ? 'Terminer la formation' : 'Marquer comme terminé →')}
                </button>
                <button onClick={() => { if (currentIdx < allContents.length - 1) navigateTo(allContents[currentIdx + 1].id); }} disabled={currentIdx >= allContents.length - 1} style={{ padding: '14px 24px', border: '2px solid #e5e7eb', borderRadius: '12px', background: 'white', color: '#4b5563', fontWeight: 600, fontSize: '15px', cursor: currentIdx >= allContents.length - 1 ? 'not-allowed' : 'pointer', opacity: currentIdx >= allContents.length - 1 ? 0.4 : 1 }}>
                  Suivant →
                </button>
              </div>
              ...
              {allDone && (
                <div style={{ padding: '32px', backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '16px', textAlign: 'center' }}>
                  ...
                  {formation.evaluation && (
                    <Link href={`/evaluation/${formation.evaluation.id}?enrollmentId=${enrollment.id}&formationId=${formationId}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 32px', backgroundColor: '#16a34a', color: 'white', borderRadius: '12px', fontWeight: 700, fontSize: '16px', textDecoration: 'none' }}>
                      <Award size={22} />Passer l'évaluation finale
                    </Link>
                  )}
                </div>
              )}
...
export default function CoursPlayerClient() {
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Loader2 style={{ width: 40, height: 40, color: '#2563eb' }} />
      </div>
    }>
      <CoursPlayer />
    </Suspense>
